  ...
  return (
    <>
      <button
        ...
        className="group flex max-w-24 items-center gap-1.5 border-2 border-ink bg-surface px-2 py-1.5 text-sm text-ink shadow-card transition duration-150 hover:bg-accent-soft active:translate-x-0.5 active:translate-y-0.5 active:shadow-none sm:max-w-44"
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block truncate text-left font-semibold">
            {activeCity ? activeCity.cityName : 'Choose location'}
          </span>
          {activeCity && activeLocality && (
            <span className="block truncate text-left text-[11px] font-medium text-muted">{activeLocality}</span>
          )}
        </span>
